...
namespace Zubair\ContactsBundle\Controller\Rest;
...
use Zubair\UtilBundle\Base\Util;
...
class ApiController extends FOSRestController
{
    ...
    public function getContactsAction(Request $request){
        ...
        foreach($entity as $obj){
            ...
            //$office = $em->getRepository("ZubairContactsBundle:Office")->find($obj->getId());
    ...
    public function getDesignationsAction(Request $request){
        ...
        $em = $this->getDoctrine()->getManager();
        $entity = $em->getRepository("Zubair\ContactsBundle\Entity\Designation")->findAll();
        $outputArray = array();
    ...
    public function saveBulkSheetAction(Request $request){
        ...
        if(count($dataArray) > 0) {
            ...
            foreach ($dataArray as $columnNo => $rowArray) {
                $empObject = new \Zubair\ContactsBundle\Entity\Employee();
            ...
            foreach ($dataArray as $columnNo => $rowArray) {
                foreach($mappedColumns as $mapColumnNo => $columnName){
                    ...
                    $empObject = $em->getRepository("Zubair\ContactsBundle\Entity\Employee")->find($empRefArray[$columnNo]);
                    if($columnName == 'emails') {
                        ...
                        foreach ($emailsArray as $element) {
                            ...
                            if (array_key_exists($emailType, $emailShortArray)) {
                                $emailObject = new \Zubair\ContactsBundle\Entity\EmployeeEmail();
                    ...
                    if($columnName == 'contacts'){
                        ...
                        foreach($contactsArray  as $element){
                            ...
                            if(array_key_exists($contactType,$contactShortArray)){
                                $contactObject = new \Zubair\ContactsBundle\Entity\EmployeeContacts();
